...
public class OptimizationTest extends TestBase {
	...
	public void optimizationTest(String date, String description, String pickUpAddress, String zone, String skill,
			...
			String vehicleNumber, String startAddress, String driverName, String idNumber) throws InterruptedException {
		...
		WebElement zoneSelInput = (WebElement) executor.executeScript("return arguments[0].parentNode",
				zoneElement);
		...
		wait = new WebDriverWait(wbDv, 10);

		// First click on dropdown to show options
		WebElement zoneSelDropdown = wait.until(ExpectedConditions.presenceOfElementLocated(By.id("select2-drop")));
		zoneSelDropdown.click();
		...
		//Assert.assertTrue(WebUIAutomation.setText("Txt_DriverName_AddVehicleScreen", name), "Unable to provide the name.");			
		
		// Add new driver name 
		
		WebElement driverNameElement = wbDv.findElement(By.name("DriverName"));		
		JavascriptExecutor driverDropdown = (JavascriptExecutor)wbDv;
		WebElement driverDropdownInput = (WebElement)driverDropdown.executeScript("return arguments[0].parentNode", driverNameElement);
		List<WebElement> driverChildInputs = driverDropdownInput.findElements(By.xpath("//input[@class='select2-input select2-default']"));
		
		driverChildInputs.get(0).click();

		if (driverName != null && !driverName.isEmpty()) {
			
			driverName = driverName + Helper.getUniqueValue().toString().substring(5);
			
			driverChildInputs.get(0).sendKeys(driverName);
			Thread.sleep(500);
			driverChildInputs.get(0).sendKeys(Keys.ARROW_DOWN);
			driverChildInputs.get(0).sendKeys(Keys.ENTER);
		} else {
			...
			// First click on dropdown to show options
			WebElement driverSelDropdown = wait.until(ExpectedConditions.presenceOfElementLocated(By.id("select2-drop")));
			driverSelDropdown.click();
		}		

		// Provide Id number
		
		UniqueIdNumber = UniqueIdNumber + Helper.getUniqueValue().toString().substring(3);
