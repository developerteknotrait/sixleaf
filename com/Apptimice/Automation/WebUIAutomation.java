...
public class WebUIAutomation extends TestBase {
	...
	public static String getMsg(String xpathKey){
		try{
		String text	= getObject(xpathKey).getText();
		return text;
		} catch (Exception e){
			return null;
		}
	...
	/**
	 * This function is used to check the value Run mode column inside the Test
	 * Case Excel file
	 * 
	 * @param xls
	 *            instance of Test Case Excel file
	 * @param testCase
	 *            Name of the test case for which Run mode value needs to check
	 * @return boolean - true/false
	 */
	public static boolean checkTestCaseRunmode(String testCase) {

		for (int i = 2; i <= datatable.getRowCount("apptimice"); i++) 
		{

			if (datatable.getCellData("apptimice", "Test Case ID",i).equalsIgnoreCase(testCase)) {

				if (datatable
						.getCellData("apptimice", "Runmode", i)
						.equalsIgnoreCase("Y")) {
